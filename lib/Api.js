...
class Api extends Homey.SimpleClass {
        ...
        await this.homey.cloud
            ...
            .then((cloudId) => {
                ...
                this._verifyAuthentication()
                    .then(() => {
                        this.log(
                            'Api.js:                    initialising done ========='
                        );
                    })
                    .catch((error) => {
                        this.log(
                            '_verifyAuthentication:     Failed:',
                            error.toString()
                        );
                        this.log(
                            'Api.js:                    initialising done ========='
    ...
    async _connectRingAPI() {
        ...
        if (this.ringApi) {
            this.log(
                '_connectRingAPI:           ring-client-api already connected, exit _connectRingAPI'
            );
            return;
        }
        ...
        try {
            ...
            this.log(
                '_connectRingAPI:           Error initializing ring-client-api',
                error
            );
            return;
        }
        ...
        this.log(
            '_connectRingAPI:           subscribing to Refresh Token updates'
        );
        ...
        try {
            ...
            this.log(
                "_connectRingAPI:           Locations retrieved, Let's see which devices can be found"
            );
            ...
            this.ringApi = null;
            if (
                error.toString().includes('Error: Refresh token is not valid.')
            ) {
                this._authenticated = false;
        ...
        // Get all cameras
        this.log(
            '_connectRingAPI:           Subscribing to camera events for all locations'
        );
    ...
    async _initializeRingApi() {
        ...
        try {
            ...
            this.log(
                '_initializeRingApi:        ring-client-api connected:',
                this.ringApi.restClient.baseSessionMetadata
            );
            this._authenticated = true;
        } catch (error) {
            this.log(
                '_initializeRingApi:        Error connecting ring-client-api',
                error
            );
    ...
    async _subRefreshTokenUpdates() {
        ...
        try {
            ...
            this.log(
                '_subRefreshTokenUpdates:   Refresh Token updates subscribed'
            );
        } catch (error) {
            this.log(
                '_subRefreshTokenUpdates:   Error onRefreshTokenUpdated',
                error
            );
        }
    }
    ...
    async _subLocations(locations) {
        ...
        for (const location of locations) {
            ...
            this.log(
                '_subLocations:             Subscribing to chime events for location:',
                location.name
            );
            await this._getChimes(location);

            this.log(
                '_subLocations:             Subscribing to intercom events for location:',
                location.name
            );
            ...
            */
            ...
            if (location.hasAlarmBaseStation) {
                this.log(
                    '_subLocations:             Ring Alarm found at',
                    location.locationDetails.name,
                    'subscribing to Alarm Mode events'
                );
                // When a location has a Ring Alarm System, subscribe to the Alarm Mode
                await this._subscribeToAlarmMode(location);
                this.log(
                    '_subLocations:             Subscribing to Alarm Device events for location:',
                    location.name
                );
                await this._getAlarmDevices(location);
            } else {
                this.log(
                    '_subLocations:             No Ring Alarm found at',
                    location.locationDetails.name
                );
    ...
    async _subscribeToAlarmMode(location) {
        ...
        this.homey.setInterval(
            () => this._refreshAlarmMode(location.locationDetails.location_id),
            alarmModeInterval * 1000
        );
    ...
    async _getAlarmDevices(location) {
        ...
        try {
                ...
                if (alarmDeviceList.indexOf(device.data.deviceType) >= 0) {
                    ...
                    try {
                        device.onData.subscribe((data) => {
                            ...
                            this.log(
                                '🔔 Raw alarm device data received:',
                        ...
                        this.log(
                            '_getAlarmDevices:          Error subscribing to Alarm Devices'
                        );
                    }
                    // Identify Alarm Security Panel
                    if (
                        device.data.deviceType == RingDeviceType.SecurityPanel
                    ) {
                        this.homey.app.alarmSystem.catalogId =
                            device.data.catalogId;
                        this.homey.app.alarmSystem.location.id = location.id;
                        this.homey.app.alarmSystem.location.name =
                            location.name;
                    }
                }
            } //)
            ...
            this.log(
                '_getAlarmDevices:          Error _getAlarmDevices',
                error
            );
        }
    }
        ...
        try {
            location.onLocationMode.subscribe((locationMode) => {
                ...
                try {
                    ...
                    this.log(
                        '_subscribeToLocationMode:  Error:',
                        location.name,
                        error
                    );
                }
            ...
            this.log(
                '_subscribeToLocationMode:  Error setting up subscription:',
                location.name,
                error
            );
        }
            ...
            response.on('end', () => {
                ...
                if (response.statusCode >= 400) {
                    if (response.statusCode == 412) {
                        ...
                        data = data.replace(/(\r\n|\n|\r)/gm, '');
                        this.log(
                            '_https_auth_cred : require mfa code : ',
                            response.statusCode
                        );
                        try {
                            report = JSON.parse(
                        ...
                        data = data.replace(/(\r\n|\n|\r)/gm, '');
                        this.log(
                            '_https_auth_cred : authentication error : ',
                            response.statusCode
                        );
                        error = new Error(
        ...
        let request = https.request(url, (response) => {
            ...
            response.on('end', () => {
                ...
                if (response.statusCode >= 400) {
                    ...
                    data = data.replace(/(\r\n|\n|\r)/gm, '');
                    this.log(
                        '_https_auth_code : invalid_authentication : ',
                        response.statusCode
                    );
                    error = new Error(
                        'invalid_authentication ' +
                    ...
                    try {
                        result = JSON.parse(data);
                        this.log(
                            '_https_auth_code: retrieved the refresh and access token'
                        );
    ...
    _onSetSettings(name) {
        ...
        if (name === 'isRevoked') {
            if (this.homey.settings.get(name)) {
                ...
                this._authenticated = false;
                this.log(
                    '_onSetSettings: Authentication revoked from settings'
                );
                //let logLine = " Api.js || " + "_onSetSettings || " + " Authentication revoked from settings ";
                //this.homey.app.writeLog(logLine);
                let report = JSON.parse(
                    '{"error": "Authentication Revoked", "error_description": "The authentication has been revoked."}'
                );
    ...
    async _verifyAuthentication() {
        ...
        if (this._authenticated) {
            ...
            const report = JSON.parse(
                '{"error": "Unauthenticated", "error_description": "The app is not authenticated at Ring."}'
            );
            this._setAuthenticationStatus(false, report);
    ...
    _setAuthenticationStatus(status, report) {
        ...
        if (this._previousAuthenticated != this._authenticated) {
            if (this._authenticated) {
                if (!this.NoAuthStatLogTimeout) {
                    this.log(
                        '_setAuthenticationStatus:  Successfully Authenticated'
                    );
                ...
                this.homey.api.realtime('com.ring.status', {
                    state: 'authenticated',
                });
        ...
        if (status) {
            if (
                this.homey.settings.get('authenticationStatus') !=
                'Authenticated'
            ) {
                this.homey.settings.set(
                    'authenticationStatus',
            ...
            if (
                this.homey.settings.get('authenticationStatus') != report.error
            ) {
    ...
    ringChime(device_data, sound, callback) {
        ...
        this.chimes.forEach(async (chime) => {
            if (chime.id == device_data.id) {
                if (sound == 'ring') {
                    await this.chimes[this.chimes.indexOf(chime)].playSound(
                        'ding'
                    );
                } else {
                    await this.chimes[this.chimes.indexOf(chime)].playSound(
    ...
    async grabImage(device_data) {
        ...
        try {
            const camera = this.cameras.find(
                (c) => c.initialData.id === device_data.id
            );
            if (!camera) {
                throw new Error(
                    `Api.js grabImage: Camera not found for device ID: ${device_data.id}`
                );
            }
    ...
    async grabVideo(device_data, offerSdp) {
        ...
        try {
            ...
            this.log(
                'Api.js grabVideo Camera is mains powered:',
                this._isCameraMainsPowered(camera)
            );
    ...
    _isCameraMainsPowered(camera) {
        ...
        if (
            typeof health.transformer_voltage === 'number' &&
            health.transformer_voltage > 16
        )
            return true;
        if (
            typeof health.transformer_status === 'string' &&
            health.transformer_status.toLowerCase() !== 'none'
        )
            return true;

        // 3) ext_power_state (some cameras)
        if (
            data.ext_power_state !== undefined &&
            data.ext_power_state !== null
        ) {
        ...
        if (
            (camera.batteryLife !== undefined ||
                camera.batteryStatus !== undefined ||
                data.battery_percentage !== undefined) &&
            health.ac_power !== 1 &&
            health.external_connection !== true
        ) {
            return false;
        }
    ...
    _reorderAndFixSdp(offerSdp, answerSdp) {
        ...
        const synthesizeApplicationBlock = (offerBlock, mid) => {
            ...
            const sctpMatch =
                offerBlock && offerBlock.match(/a=sctp-port:(\d+)/);
        ...
        for (const mid of offerMids) {
            ...
            if (block) {
                ...
                if (offerDir === 'recvonly') {
                    ...
                    if (
                        !/^\s*a=sendonly\b/m.test(block) &&
                        !/^\s*a=sendrecv\b/m.test(block)
                    ) {
                        // nothing
                    }
                    // replace sendrecv with sendonly, or ensure sendonly present
                    block = block.replace(
                        /\b(sendrecv|recvonly|sendonly)\b/,
                        'sendonly'
                    );
                }
                // ensure end-of-candidates present after candidates in each block (if candidates exist)
                if (
                    /^a=candidate:/m.test(block) &&
                    !/a=end-of-candidates/m.test(block)
                ) {
                ...
                if (offerBlock && /^m=application\b/m.test(offerBlock)) {
                    ...
                    block = [
                        `${mline.split(' ')[0]} 0 ${mline.split(' ')[2]} ${mline
                            .split(' ')
                            .slice(3)
                            .join(' ')}`, // keep payload types
                        `c=IN IP4 0.0.0.0`,
                        `a=mid:${mid}`,
                        `a=inactive`,
                    ].join('\n');
        ...
        const offerBundle =
            (offer.header.match(/^a=group:BUNDLE (.+)$/m) || [])[1] ||
            offerMids.join(' ');
        const headerLines = offer.header
            .split('\n')
            .filter(Boolean)
            .map((l) => l.trim());
        ...
        if (/^a=group:BUNDLE /m.test(answerHeader)) {
            answerHeader = answerHeader.replace(
                /^a=group:BUNDLE .*/m,
    ...
    enableMotion(device_data, callback) {
        ...
        this.cameras.forEach(async (camera) => {
            if (camera.initialData.id == device_data.id) {
                ...
                const result = await this.cameras[
                    this.cameras.indexOf(camera)
                ].setDeviceSettings(newSetting);
                const mAlerts = await this.cameras[
                    this.cameras.indexOf(camera)
            ...
            if (location.locationDetails.location_id == locationId) {
                ...
                if (this.location.hasAlarmBaseStation) {
                    ...
                    try {
                        ...
                        result = await this.locations[
                            locationIndex
                        ].setAlarmMode(alarmModes[mode]);
                        callback(null, result);
                    } catch (error) {
