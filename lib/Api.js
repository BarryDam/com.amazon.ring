...
let RingApi, RingDeviceType;

try {
    // Check if ReadableStream is available (Homey Pro Early 2023 and later)
    // if (typeof ReadableStream !== 'undefined') {
    RingApi = require('../ring-client-api').RingApi;
    RingDeviceType = require('../ring-client-api').RingDeviceType;
    console.log('Using latest ring-client-api');
    // } else {
    //     // no ReadableStream, so old Homey
    //     throw new Error('ReadableStream not supported');
    // }
} catch (err) {
    console.warn('Falling back to ring-client-api 13.1.0 due to:', err.message);
    RingApi = require('../ring-client-api13.1.0').RingApi;
    RingDeviceType = require('../ring-client-api13.1.0').RingDeviceType;
}
        ...
        if (name === 'isRevoked') {
            if (this.homey.settings.get(name)) {
                ...
                let logLine =
                    ' Api.js || ' +
                    '_onSetSettings || ' +
                    ' Authentication revoked from settings ';
