...
class Api extends Homey.SimpleClass {
                    ...
                    */
                    if (device.data.deviceType == RingDeviceType.SecurityPanel) {
                        this.homey.app.alarmSystems.push({
                            ...
                            mode: null // optional, will be updated later
                        });
                    }
                }
            }//)
    ...
    async _getCamerasDoorbells(location) {
        ...
        try {
            ...
            this.log('Api.js _getCamerasDoorbells error:',error.toString());
            throw error;
        }
    }
    ...
    async _getChimes(location) {
        ...
        try {
            ...
            /*
            for (const chime of locationChimes) {
   ...
   async _refreshAlarmMode (locationId) {
    ...
    _https_auth_cred (auth, callback) {
        ...
        const url = new URL('https://oauth.ring.com/oauth/token');
    ...
    async getAlarmDevices(callback) {
        try {
            ...
            for (const location of this.locations) {  // assuming this.locations is an array
...
// this.log(camera.data);
    ...
    // todo: remove callbacks from methods and improve error handling
        ...
        this.cameras.forEach(async (camera) => {
            if (camera.initialData.id == device_data.id) {
                try {
                    ...
                    const mAlerts = await camera.subscribeToMotionEvents();
                    callback(null, result);
                } catch (error ) {
        ...
        this.cameras.forEach(async (camera) => {
            if (camera.initialData.id == device_data.id) {
                try {
                    ...
                    const result = await camera.setDeviceSettings(newSetting);
                    const mAlerts = await camera.unsubscribeFromMotionEvents();
                    callback(null, result);
                } catch (error) {
